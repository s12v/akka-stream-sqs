...
import com.amazonaws.services.sqs.model.{Message, ReceiveMessageRequest, SendMessageRequest, SendMessageResult}
...
class SqsPublishSinkShapeSpec extends FlatSpec with Matchers with DefaultTestContext {
  it should "send a message" in {
    ...
    val sendMessageRequest = new SendMessageRequest().withMessageBody("dssdhk")
    ...
    probe.sendNext(sendMessageRequest)
